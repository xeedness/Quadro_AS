...
uint8_t sendPacketTwi1(uint8_t slave_address, uint8_t address, uint32_t size, uint8_t* data) {
	uint8_t res = sendPacket(TWI1, slave_address, address, data, size);
	if(res > 0) {
		return 0;
	} else {
		return 1;
...
uint32_t receivePacketTwi1(uint8_t slave_address, uint8_t address, uint32_t size, void* data) {
	uint32_t res = receivePacket(TWI1, slave_address, address, data, size);
	if(res > 0) {
		return 0;
	} else {
		return 1;
