...
void calibrate(uint16_t* gyroBias, uint16_t* accelBias) {
	...
	gyroBias[0] = gyro_bias[0]; // construct gyro bias in deg/s for later manual subtraction
...
void setupSensor() {
	...
	printf("AccelSelfTest: %d %d %d\n",(uint32_t)(100*SelfTest[0]), (uint32_t)(100*SelfTest[1]), (uint32_t)(100*SelfTest[2]));
	printf("GyroSelfTest: %d %d %d\n",(uint32_t)(100*SelfTest[3]), (uint32_t)(100*SelfTest[4]), (uint32_t)(100*SelfTest[5]));
...
uint32_t printCounter = SAMPLES_PER_SECOND;
uint8_t updateOrientation() {
	...
	if(++sumCounter < 1000/SAMPLES_PER_SECOND) {
		...
		if(--printCounter == 0) {
			printCounter = SAMPLES_PER_SECOND;
